This file path was: 'app/caretaker/dashboard/risk-profile/page.js'

"use client";
import { useState, useEffect } from "react";
import CaretakerSidebar from "../../../Components/CaretakerSidebar";
import { RefreshCw, Heart, Activity, Zap, Wind, Shield, Dumbbell, Droplets, Eye, Lock, ChevronDown, ChevronUp, AlertTriangle, Stethoscope, Baby, Dna, AlertCircle, ShieldAlert, Ear } from "lucide-react";
        ...
        <div className="mt-4 pt-4 border-t border-white/50 dark:border-white/10 space-y-4">
          {reasons.length > 0 && (
            <p className="text-[13px] text-neutral-600 dark:text-neutral-300 leading-relaxed">
              The main reasons this showed up are{' '}
              <span className="font-semibold text-neutral-800 dark:text-neutral-100">
                {reasons.length === 1
                  ? reasons[0]
                  : reasons.slice(0, -1).join(', ') + ' and ' + reasons[reasons.length - 1]}
              </span>.
              {' '}Worth discussing with their doctor.
            </p>
          )}
          {breakdown.length > 0 && (
      ...
      {!isPaid && isMonitored && (
        <div className="mt-3 pt-3 border-t border-white/50 dark:border-white/10">
          <p className="text-[12px] text-gray-500 dark:text-neutral-400">
            <Link href="/caretaker/dashboard/subscription" className="font-semibold text-blue-600 dark:text-blue-400 underline">
              Upgrade
            </Link>{' '}
            to see why this area showed up.
          </p>
        </div>
      )}
    </div>
...
export default function CaretakerRiskProfilePage() {
  ...
  useEffect(() => {
    ...
    setCaretakerId(cid);
    fetch(`/api/subscription?patientId=${cid}`)
      .then(r => r.json()).then(d => setIsPaid(!!d.plan)).catch(() => {});
    loadProfile(cid);
  }, []);

  function loadProfile(cid) {
    const id = cid || caretakerId;
    setLoading(true);
    fetch(`/api/caretaker/risk-profile?caretakerId=${id}`)
      .then(r => r.json())
  ...
  async function refresh() {
    ...
    setRefreshing(true);
    try {
      await fetch('/api/caretaker/risk-profile', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ caretakerId }),
      });
      loadProfile(caretakerId);
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div>
              ...
              <p className="text-gray-500 dark:text-neutral-400 text-sm mt-2 max-w-lg">
                Based on the patient's health profile — areas worth keeping an eye on. Not a diagnosis.
              </p>
            </div>
            <button
              onClick={refresh}
          ...
          {loading ? (
            ...
            <div className="border border-gray-200 dark:border-white/8 rounded-2xl p-10 text-center">
              ...
              <p className="text-sm text-gray-400 dark:text-neutral-500 mt-1 mb-4">
                The patient's personal details need to be completed first.
              </p>
              <Link
                href="/caretaker/dashboard/personal-details"
            ...
            <>
              <div className="rounded-2xl p-5 bg-gradient-to-br from-[#1E4E8C] to-[#2d6cc0] text-white flex items-center justify-between flex-wrap gap-4">
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-widest opacity-70 mb-1">Patient Summary</p>
              ...
              <p className="text-xs text-gray-400 dark:text-neutral-600 leading-relaxed text-center max-w-xl mx-auto">
                These signals are based on population-level health data and the patient's profile. Not a diagnosis.
